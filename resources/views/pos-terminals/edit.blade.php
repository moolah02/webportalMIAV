                ...
                <div class="mb-8 pb-6 border-b border-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                        <div>
                            ...
                            <select name="region" id="edit_province_select" class="ui-select" onchange="document.getElementById('edit_province_hidden').value=this.value">
                                <option value="">Select Province</option>
                                @foreach($regions as $reg)
                                    <option value="{{ $reg }}" {{ old('region', $posTerminal->region) == $reg ? 'selected' : '' }}>{{ $reg }}</option>
                                @endforeach
                            </select>
                            <input type="hidden" name="province" id="edit_province_hidden" value="{{ old('province', $posTerminal->province ?? $posTerminal->region) }}">
                        </div>
                        <div>
