...
Route::middleware(['auth', 'active.employee'])->group(function () {
    ...
    // ==============================================
    // ASSET MANAGEMENT ROUTES (Admin/Manager)
    // ==============================================
    Route::middleware('permission:manage_assets,all')->prefix('admin/assets')->name('assets.')->group(function () {
    ...
    // ==============================================
    // ASSET REQUEST ROUTES (All Employees - Shopping Cart)
    // ==============================================
    Route::prefix('asset-requests')->name('asset-requests.')->group(function () {
        ...
        // Checkout and Request Management
...
// Asset Approval Routes (Manager/Admin Only)
Route::middleware(['auth', 'can:manage_assets'])->prefix('asset-approvals')->name('asset-approvals.')->group(function () {
...
// Also make sure you have these existing routes for asset requests:
Route::middleware(['auth', 'active.employee'])->prefix('asset-requests')->name('asset-requests.')->group(function () {
    ...
    // ==============================================
    // ASSET APPROVAL ROUTES (Managers/Admin)
    // ==============================================
    Route::middleware('permission:manage_assets,all')->prefix('asset-approvals')->name('asset-approvals.')->group(function () {
        Route::get('/', [AssetApprovalController::class, 'index'])->name('index');
        Route::get('/{assetRequest}', [AssetApprovalController::class, 'show'])->name('show');
        Route::post('/{assetRequest}/approve', [AssetApprovalController::class, 'approve'])->name('approve');
        Route::post('/{assetRequest}/reject', [AssetApprovalController::class, 'reject'])->name('reject');
        Route::post('/{assetRequest}/fulfill', [AssetApprovalController::class, 'fulfill'])->name('fulfill');
        Route::post('/bulk-action', [AssetApprovalController::class, 'bulkAction'])->name('bulk-action');
    });
...
Route::middleware(['auth', 'can:manage_assets'])->prefix('assets')->name('assets.')->group(function () {
...
// Add this temporary route for assets
Route::get('/assets', function() {
    return view('assets.index', [
        'assets' => collect([]), // Empty collection for now
        'categories' => collect([]),
    ]);
})->name('assets.index');